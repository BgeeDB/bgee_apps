...
public class GenerateExprFile extends GenerateDownloadFile {
     ...
     *                                      are the conditions. Calls not in these conditions, 
     *                                      will be discarded. If it is {@code null}, 
     *                                      there is no filter.
     * @return                              A {@code Map} where keys are {@code String}s that 
     *                                      are gene IDs, the associated values being a {@code Set} 
    ...
    private Map<String, Set<NoExpressionCallTO>> loadNoExprCallsByGeneIds(Set<String> speciesIds, 
        ...
        try (NoExpressionCallTOResultSet rsNoExpr = dao.getNoExpressionCalls(params)) {
            while (rsNoExpr.next()) {
                ...
                if (allConditions != null && !allConditions.contains(new SingleSpeciesCondition(to))) {
    ...
    private void generateExprFilesForOneSpecies(String fileNamePrefix, 
        ...
        Set<SingleSpeciesCondition> propagatedExprConditions = this.propagateConditions(
        ...
        Set<SingleSpeciesCondition> propagatedNoExprConditions = this.propagateConditions(
                this.getConditions(noExprTOsByGeneIds), null, anatEntityParentsFromChildren);
        ...
        allConditions.addAll(propagatedNoExprConditions);
