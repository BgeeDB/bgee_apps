 ...
 * <p>
 * A <code>DiffExpressionAnalysis</code> compare different conditions, according 
 * to a specified {@link ComparisonFactor} (see {@link #getComparisonFactor()}). 
 * For instance, if the comparison factor is <code>ANATOMY</code>, it means that different 
...
public abstract class DiffExpressionAnalysis {
	 ...
	 * <ul>
	 * <li><code>ANATOMY</code>: the analysis compared expression in different 
	 * anatomical structures, at an equivalent (broad) developmental stage.
	 * <li><code>DEVELOPMENT</code>: the analysis compared expression in a same 
